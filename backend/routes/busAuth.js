const express = require('express');
const router = express.Router();
const BusUser = require('../models/BusUser');
const BusSession = require('../models/BusSession');
const mongoose = require('mongoose');
const jwt = require('jsonwebtoken');

/**
 * Bus Login Route
 *
 * Enforces a "Block if Active" policy.
 * If an active session already exists for this account, the new login attempt
 * is rejected with a warning message.
 */
router.post('/login', async (req, res) => {
    const { busNumber, password } = req.body;

    console.log(`\n--- Login Request: Bus ${busNumber} ---`);

    try {
        // 1. Authenticate the user
        const user = await BusUser.findOne({ busNumber });

        if (!user || user.password !== password) {
            console.log('Login Failed: Credentials mismatch');
            return res.status(401).json({ message: 'Invalid bus number or password' });
        }

        // 2. SINGLE DEVICE ENFORCEMENT (Block strategy)
        const activeSession = await BusSession.findOne({ busNumber: busNumber, isActive: true });

        if (activeSession) {
            console.log(`Login Blocked: Bus ${busNumber} is already active on another device.`);
            return res.status(409).json({
                message: 'This account is already in use on another device. Please logout from the first device before logging in here.'
            });
        }

        // 3. Create a unique JWT
        const token = jwt.sign(
            {
                userId: user._id,
                busNumber: user.busNumber,
                vehicleId: user.vehicleId,
                sessionStarted: new Date().getTime()
            },
            process.env.JWT_SECRET,
            { expiresIn: '12h' }
        );

        // 4. Save the new session
        const newSession = new BusSession({
            userId: user._id,
            busNumber: user.busNumber,
            token: token,
            isActive: true
        });
        await newSession.save();

        console.log(`Login Successful: Session started for Bus ${busNumber}`);

        res.status(200).json({
            message: 'Login successful',
            token: token,
            user: {
                busNumber: user.busNumber,
                vehicleId: user.vehicleId,
                depotLocation: user.depotLocation
            }
        });
    } catch (error) {
        console.error('Login System Error:', error.message);
        res.status(500).json({ message: 'Server error', error: error.message });
    }
});

/**
 * Bus Logout Route
 */
const busAuth = require('../middleware/busAuth');
router.post('/logout', busAuth, async (req, res) => {
    const token = req.token;

    try {
        await BusSession.findOneAndUpdate(
            { token: token },
            { isActive: false, logoutTime: new Date() }
        );
        console.log(`Session explicitly logged out.`);
        res.status(200).json({ message: 'Logout successful' });
    } catch (error) {
        res.status(500).json({ message: 'Logout error' });
    }
});

module.exports = router;
